            ...
            if (resultCode == RESULT_OK) {
                if(cameraFileUri.value() != null) {
                    ...
                    if(bitmap.getHeight() > bitmap.getWidth()){
                        bmpRatio = REDUCE_PIC_SIZE/(float)bitmap.getHeight();
                    }
                    else{
                        bmpRatio = REDUCE_PIC_SIZE/(float)bitmap.getWidth();
                    }
                    matrix.postScale(bmpRatio, bmpRatio);
                    Bitmap bitmapSmall = bitmap.createBitmap(bitmap, 0, 0, bitmap.getWidth(),
